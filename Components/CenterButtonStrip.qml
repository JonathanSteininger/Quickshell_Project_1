...
Canvas{
    //searches for the child that has a center property set as true;
    id: canvas
    ...
    onPaint: {
        ...
        */
        ...
        for (var i = 2; i < children.length;i++){
            drawLine(ctx, children[i].x - spacing/2, borderSize, borderColor, i - 1 > center);
        }
    }

    function drawLine(ctx, xpos,  borderSize:real, borderColor, flipped:bool){
        ctx.beginPath();
        ctx.strokeStyle = borderColor
        ctx.lineWidth = Math.ceil(borderSize);
        if(tiltRight != flipped){
    ...
    function getChildBounds(childIndex: int): list<var>{
        ...
        if (childIndex < center + 1){
            return createParallelogram(_left, 0, _span);
        } 
        if (childIndex == center + 1){
            return createTrapazoid(_left, 0, _span);
        }
        return createParallelogramFlipped(_left, 0, _span);
    }
    ...
    function createParallelogramFlipped(
        _x = this.height/2,
        _y = 0,
        _width = this.width-this.height,
        _height = this.height,
